...
export default class CandidatesAndVotes extends LitElement {
  ...
  constructor() {
    ...
    uSElectionsService.democratCandidate.subscribe(x => { this.democratCandidate = x; this.requestUpdate(); });
    uSElectionsService.electoralVotesDemocrat.subscribe(x => { this.electoralVotesDemocrat = x; this.requestUpdate(); });
    uSElectionsService.popularVotesDemocrat.subscribe(x => { this.popularVotesDemocrat = x; this.requestUpdate(); });
    uSElectionsService.republicanCandidate.subscribe(x => { this.republicanCandidate = x; this.requestUpdate(); });
    uSElectionsService.electoralVotesRepublican.subscribe(x => { this.electoralVotesRepublican = x; this.requestUpdate(); });
    uSElectionsService.popularVotesRepublican.subscribe(x => { this.popularVotesRepublican = x; this.requestUpdate(); });
    stateService.currentlyChosenYear.pipe(takeUntil(this.destroy$)).subscribe(() => { uSElectionsService.getVoteResultList(stateService.currentlyChosenYear.value as any).then((data) => {
        this.uSElectionsVoteResult = data;
      });
    });
    stateService.currentlyChosenYear.pipe(takeUntil(this.destroy$)).subscribe(() => { uSElectionsService.getVoteResultList1(stateService.currentlyChosenYear.value as any).then((data) => {
        this.uSElectionsVoteResult1 = data;
      });
    });
    stateService.currentlyChosenYear.pipe(takeUntil(this.destroy$)).subscribe(() => { uSElectionsService.getCandidateVoteResultList(stateService.currentlyChosenYear.value as any).then((data) => {
        this.uSElectionsCandidateVoteResult = data;
      });
    });
    stateService.currentlyChosenYear.pipe(takeUntil(this.destroy$)).subscribe(() => { uSElectionsService.getStateVoteResultList(stateService.currentlyChosenYear.value as any).then((data) => {
        this.uSElectionsStateVoteResult = data;
      });
    });
  }
  ...
  @state()
  private uSElectionsVoteResult: VoteResult[] = [];

  @state()
  private uSElectionsVoteResult1: VoteResult[] = [];

  @state()
  private uSElectionsCandidateVoteResult: CandidateVoteResult[] = [];

  @state()
  private uSElectionsStateVoteResult: StateVoteResult[] = [];
    ...
    return html`
      ...
      <div class="column-layout group">
        ...
        <div class="row-layout pies_and_grid">
          <div class="row-layout pie_charts">
            <div class="column-layout group_12">
              ...
              <igc-pie-chart .dataSource="${this.uSElectionsVoteResult}" start-angle="90" label-inner-color="#FAFAFA" others-category-threshold="0.5" label-member-path="resultAsString" value-member-path="resultAsNumber" class="pie-chart"></igc-pie-chart>
            </div>
            <div class="column-layout group_12">
              <p class="typography__subtitle-2 content">
                Popular Vote
              </p>
              <igc-pie-chart .dataSource="${this.uSElectionsVoteResult1}" start-angle="90" label-inner-color="#FAFAFA" others-category-threshold="0.5" label-member-path="resultAsString" value-member-path="resultAsNumber" class="pie-chart"></igc-pie-chart>
            </div>
          </div>
          <div class="column-layout group_13">
            <igc-grid .data="${this.uSElectionsCandidateVoteResult}" primary-key="party" ?allow-filtering="${true}" filter-mode="excelStyleFilter" class="ig-typography ig-scrollbar grid">
        ...
        <div class="column-layout group_13">
          ...
          <igc-category-chart .dataSource="${this.uSElectionsStateVoteResult}" chart-type="column" x-axis-label-angle="60" computed-plot-area-margin-mode="series" class="category-chart"></igc-category-chart>
